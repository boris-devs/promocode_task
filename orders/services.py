...
def _validate_promocode(promo_code: str, user_id: dict) -> Promocode:
    promo = (
        Promocode.objects.select_for_update()
        .filter(code=promo_code, active=True)
        .first()
    )
    if not promo:
    ...
    if PromocodeUsage.objects.filter(promo_code=promo, user=user_id).exists():
...
class OrderService:
    ...
    def create_order(order_data: dict):
        with transaction.atomic():
            ...
            for item in goods_payload:
                ...
                if quantity <= 0:
                    raise ValidationError(
                        {"goods": f"Quantity for good {good_id} must be greater than 0"}
                    )
            ...
            goods_qs = Good.objects.select_related("category").filter(
            ...
            if missing_ids:
                raise ValidationError(
                    {"goods": f"Missing goods with ids: {missing_ids}"}
                )
            ...
            if promo_code:
                promo = _validate_promocode(promo_code, user_id)
                ...
                for good in goods_by_id.values():
                    ...
                    if (
                        has_category_limit
                        and good.category_id not in allowed_category_ids
                    ):
                        continue
                ...
                if not eligible_good_ids:
                    raise ValidationError(
                        {"promo_code": "Promo code is not applicable to selected goods"}
                    )
            ...
            if promo:
                ...
                PromocodeUsage.objects.create(promo_code=promo, user=user_id)
